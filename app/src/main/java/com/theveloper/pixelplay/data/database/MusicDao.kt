...
interface MusicDao {
    ...
    @Query("SELECT id FROM songs WHERE source_type = 8")
    suspend fun getAllBilibiliSongIds(): List<Long>
    ...
    @Transaction
    suspend fun clearAllBilibiliSongs() {
        val bilibiliSongIds = getAllBilibiliSongIds()
        if (bilibiliSongIds.isEmpty()) return
        deleteSongsAndRelatedData(bilibiliSongIds)
    }
    ...
    /**
     * Finds a cloud song (source_type = 7) by matching title and artist name.
     */
    @Query("SELECT id FROM songs WHERE source_type = 7 AND title = :title AND artist_name = :artist LIMIT 1")
    suspend fun getCloudSongByTitleArtist(title: String, artist: String): Long?
