...
# We want depth at START of each line. Record depth when we are ABOUT to process a line,
# and detect declaration lines.
results = []
decl_re = re.compile(r'\b(private\s+|override\s+|internal\s+|protected\s+)?fun\s+\w+')
while i < N:
    # at start of line ln, depth is current
    d = depth
    ln = line
    # read the whole current line text for detection
    j = src.find("\n", i)
    if j == -1: j = N
    text = src[i:j]
    if decl_re.search(text) and ('fun ' in text):
        results.append((ln, d, text.strip()[:60]))
    # scan the current line char by char to update depth
    while i < N and src[i] != "\n":
        scan()
    # now at newline
    if i < N and src[i] == "\n":
...
print("Function/method declaration lines and their depth (class-member if depth==1):")
for ln, d, t in results:
    tag = "MEMBER" if d == 1 else ("NESTED" if d >= 2 else "TOP/OTHER")
